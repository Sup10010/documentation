...
You can add contents (e.g., fields, lists, tables, images, banners, etc.) to the report using
:ref:`commands <knowledge/powerbox>`. Type `/` to open the **powerbox**, then type the command's
...
.. spoiler:: Modify a non-compliant table
   ...
   find the following structure in the XML (simplified for documentation purposes):
    ...
    .. note::
       ...
       `Mozilla Developper Network website <https://developer.mozilla.org/en-US/docs/Learn/HTML/Tables/Basics>`_.
...
.. spoiler:: Conditional blocks
   ...
   The title *Pro-Forma Invoice* is used depending on some contextual conditions. Then, if these
...
.. spoiler:: Images
   ...
   For example, the following code outputs the field `image_128` of the line's product as 64px-wide
   image (with an automatic height based on the image's aspect ratio).
